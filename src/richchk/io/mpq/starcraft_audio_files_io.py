"""Add audio files to a StarCraft MPQ and update the WAV/STR sections in the CHK."""
import os
import shutil
...
class StarCraftAudioFilesIo:
    ...
    def add_audio_files_to_mpq(
        self,
        path_to_audio_files_on_disk: list[str],
        ...
        :param path_to_audio_files_on_disk:
        :param path_to_base_mpq_file:
        :param path_to_new_mpq_file:
        :param overwrite_existing:
        :return:
        """
        if not all((os.path.exists(wav) for wav in path_to_audio_files_on_disk)):
            raise FileNotFoundError(
                f"At least one WAV file does not exist: {path_to_audio_files_on_disk}"
            )
        ...
        with CrossPlatformSafeTemporaryNamedFile() as temp_mpq_file:
            ...
            new_wav_files = self._add_audio_files_to_mpq(
                path_to_mpq_file=temp_mpq_file,
                path_to_wavs_on_disk=path_to_audio_files_on_disk,
            )
    ...
    def _add_audio_files_to_mpq(
        ...
        for wavfile in path_to_wavs_on_disk:
            wavfile_in_mpq = self._create_audio_filepath_in_mpq(wavfile)
    ...
    @classmethod
    def _create_audio_filepath_in_mpq(cls, path_to_wav_on_disk: str) -> str:
    ...
    def _update_chk_wav_section(
        ...
        new_wav = self._update_wav_with_new_audio_files(new_wav_files, chk)
        new_chk = RichChkEditor().replace_chk_section(new_wav, chk)
        return new_chk

    def _update_wav_with_new_audio_files(
