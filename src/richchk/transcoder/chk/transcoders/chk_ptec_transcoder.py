"""Decode and encode PTEC - Classic Tech Restrictions.

u8[12][24] player_tech_availability u8[12][24] player_tech_researched u8[24]
global_tech_availability u8[24]     global_tech_researched u8[12][24]
...
class ChkPtecTranscoder(
    ChkSectionTranscoder[DecodedPtecSection],
    _RegistrableTranscoder,
    chk_section_name=DecodedPtecSection.section_name(),
):
    def decode(self, chk_section_binary_data: bytes) -> DecodedPtecSection:
    ...
    def _encode(self, decoded_chk_section: DecodedPtecSection) -> bytes:
        data = struct.pack(
