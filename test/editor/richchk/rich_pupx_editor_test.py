...
def test_it_sets_uses_defaults_false_on_max_level_override(default_pupx):
    editor = RichPupxEditor()
    updated = editor.set_player_max_level(
        PlayerId.PLAYER_1, UpgradeId.TERRAN_INFANTRY_ARMOR, 2, default_pupx
    )
...
def test_it_sets_uses_defaults_false_on_start_level_override(default_pupx):
    editor = RichPupxEditor()
    updated = editor.set_player_start_level(
        PlayerId.PLAYER_1, UpgradeId.TERRAN_INFANTRY_ARMOR, 1, default_pupx
    )
    assert (
        updated.player_uses_defaults[PlayerId.PLAYER_1][UpgradeId.TERRAN_INFANTRY_ARMOR]
        is False
...
def test_apply_player_max_levels_merges_partial_dict(default_pupx):
    editor = RichPupxEditor()
    updates = {
        PlayerId.PLAYER_1: {
            UpgradeId.TERRAN_INFANTRY_ARMOR: 1,
            UpgradeId.UNKNOWN_UPGRADE_60: 2,
        },
        PlayerId.PLAYER_2: {UpgradeId.TERRAN_INFANTRY_ARMOR: 0},
    }
    updated = editor.apply_player_max_levels(updates, default_pupx)
    assert (
        updated.player_max_levels[PlayerId.PLAYER_1][UpgradeId.TERRAN_INFANTRY_ARMOR]
        == 1
    )
...
def test_apply_player_start_levels_merges_partial_dict(default_pupx):
    editor = RichPupxEditor()
    updates = {
        PlayerId.PLAYER_1: {UpgradeId.TERRAN_INFANTRY_ARMOR: 1},
        PlayerId.PLAYER_2: {UpgradeId.UNKNOWN_UPGRADE_60: 2},
    }
    updated = editor.apply_player_start_levels(updates, default_pupx)
    assert (
        updated.player_start_levels[PlayerId.PLAYER_1][UpgradeId.TERRAN_INFANTRY_ARMOR]
        == 1
    )
    assert (
        updated.player_start_levels[PlayerId.PLAYER_2][UpgradeId.UNKNOWN_UPGRADE_60]
        == 2
    )
