...
from richchk.transcoder.chk.transcoders.chk_ptex_transcoder import ChkPtexTranscoder
...
def test_it_decodes_expected_player_tech_availability():
    transcoder = ChkPtexTranscoder()
    binary = _build_ptex_binary(player_avail=1)
...
def test_it_decodes_expected_global_tech_availability():
    transcoder = ChkPtexTranscoder()
    binary = _build_ptex_binary(global_avail=0)
    section = transcoder.decode(binary)
