...
from richchk.transcoder.chk.transcoders.chk_upgs_transcoder import ChkUpgsTranscoder

_NUM_UPGRADES = 46


def _build_upgs_binary(
    uses_default: int = 1,
...
def test_it_decodes_expected_uses_default_settings():
    transcoder = ChkUpgsTranscoder()
    binary = _build_upgs_binary(uses_default=1)
...
def test_it_decodes_expected_costs():
    transcoder = ChkUpgsTranscoder()
    binary = _build_upgs_binary(base_mineral=200, mineral_factor=150, base_gas=100)
    section = transcoder.decode(binary)
