...
from richchk.transcoder.richchk.transcoders.rich_dd2_transcoder import RichDd2Transcoder

from ....chk_resources import CHK_SECTION_FILE_PATHS


@pytest.fixture
def real_decoded_dd2() -> DecodedDd2Section:
    with open(
        CHK_SECTION_FILE_PATHS[DecodedDd2Section.section_name().value], "rb"
    ) as f:
        chk_binary_data = f.read()
    return ChkDd2Transcoder().decode(chk_binary_data)
...
def test_it_decodes_empty_dd2_section(real_decoded_dd2, rich_decode_context):
    rich_dd2 = RichDd2Transcoder().decode(
        real_decoded_dd2,
...
def test_integration_it_decodes_and_encodes_back_without_changing_data(
    real_decoded_dd2, rich_decode_context, rich_encode_context
):
    rich_dd2 = RichDd2Transcoder().decode(
        real_decoded_dd2,
        rich_chk_decode_context=rich_decode_context,
    )
    actual_decoded_dd2 = RichDd2Transcoder().encode(
        rich_dd2,
        rich_chk_encode_context=rich_encode_context,
    )
    rich_dd2_again = RichDd2Transcoder().decode(
        actual_decoded_dd2,
        rich_chk_decode_context=rich_decode_context,
    )
    assert rich_dd2 == rich_dd2_again
    assert actual_decoded_dd2 == real_decoded_dd2
