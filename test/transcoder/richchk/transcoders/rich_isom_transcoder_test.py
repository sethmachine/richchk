...
from richchk.transcoder.richchk.transcoders.rich_isom_transcoder import (
    RichIsomTranscoder,
)
...
@pytest.fixture
def real_decoded_isom() -> DecodedIsomSection:
    with open(
        CHK_SECTION_FILE_PATHS[DecodedIsomSection.section_name().value], "rb"
    ) as f:
        chk_binary_data = f.read()
    return ChkIsomTranscoder().decode(chk_binary_data)
...
def test_it_decodes_expected_data_length(real_decoded_isom, rich_decode_context):
    rich_isom = RichIsomTranscoder().decode(
        real_decoded_isom,
        rich_chk_decode_context=rich_decode_context,
    )
    assert len(rich_isom.data) == ((128 // 2 + 1) * (128 + 1)) * 4


def test_integration_it_decodes_and_encodes_back_without_changing_data(
    real_decoded_isom, rich_decode_context, rich_encode_context
):
    rich_isom = RichIsomTranscoder().decode(
        real_decoded_isom,
        rich_chk_decode_context=rich_decode_context,
    )
    actual_decoded_isom = RichIsomTranscoder().encode(
        rich_isom,
        rich_chk_encode_context=rich_encode_context,
    )
    rich_isom_again = RichIsomTranscoder().decode(
        actual_decoded_isom,
        rich_chk_decode_context=rich_decode_context,
    )
    assert rich_isom == rich_isom_again
    assert actual_decoded_isom == real_decoded_isom
