...
from richchk.transcoder.richchk.transcoders.rich_mask_transcoder import (
    RichMaskTranscoder,
)
...
@pytest.fixture
def real_decoded_mask() -> DecodedMaskSection:
    with open(
        CHK_SECTION_FILE_PATHS[DecodedMaskSection.section_name().value], "rb"
    ) as f:
        chk_binary_data = f.read()
    return ChkMaskTranscoder().decode(chk_binary_data)
...
def test_it_decodes_expected_fog_data_length(real_decoded_mask, rich_decode_context):
    rich_mask = RichMaskTranscoder().decode(
        real_decoded_mask,
        rich_chk_decode_context=rich_decode_context,
    )
    assert len(rich_mask.fog_data) == 128 * 128


def test_integration_it_decodes_and_encodes_back_without_changing_data(
    real_decoded_mask, rich_decode_context, rich_encode_context
):
    rich_mask = RichMaskTranscoder().decode(
        real_decoded_mask,
        rich_chk_decode_context=rich_decode_context,
    )
    actual_decoded_mask = RichMaskTranscoder().encode(
        rich_mask,
        rich_chk_encode_context=rich_encode_context,
    )
    rich_mask_again = RichMaskTranscoder().decode(
        actual_decoded_mask,
        rich_chk_decode_context=rich_decode_context,
    )
    assert rich_mask == rich_mask_again
    assert actual_decoded_mask == real_decoded_mask
