...
from richchk.model.chk.ptex.decoded_ptex_section import DecodedPtexSection
...
from richchk.transcoder.richchk.transcoders.rich_ptex_transcoder import (
    RichPtexTranscoder,
)
...
def test_it_decodes_player_tech_availability_as_dict(decode_context):
    decoded = _make_decoded_ptex(player_avail=1)
    rich = RichPtexTranscoder().decode(decoded, decode_context)
...
def test_it_decodes_bw_tech_availability(decode_context):
    decoded = _make_decoded_ptex(player_avail=1)
    rich = RichPtexTranscoder().decode(decoded, decode_context)
    assert rich.player_tech_availability[PlayerId.PLAYER_1][TechId.RESTORATION] is True


def test_it_decodes_global_tech_availability(decode_context):
    decoded = _make_decoded_ptex(global_avail=0)
    rich = RichPtexTranscoder().decode(decoded, decode_context)
    assert rich.global_tech_availability[TechId.STIM_PACKS] is False


def test_it_decodes_player_tech_researched(decode_context):
    decoded = _make_decoded_ptex(player_researched=1)
    rich = RichPtexTranscoder().decode(decoded, decode_context)
...
def test_it_encodes_and_round_trips(decode_context, encode_context):
    decoded = _make_decoded_ptex()
    transcoder = RichPtexTranscoder()
