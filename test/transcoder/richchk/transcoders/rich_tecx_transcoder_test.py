...
from richchk.model.chk.tecx.decoded_tecx_section import DecodedTecxSection
...
from richchk.transcoder.richchk.transcoders.rich_tecx_transcoder import (
    RichTecxTranscoder,
)
...
def test_it_decodes_correct_number_of_settings(decode_context):
    decoded = _make_decoded_tecx()
    rich = RichTecxTranscoder().decode(decoded, decode_context)
...
def test_it_decodes_bw_tech_ids_as_keys(decode_context):
    decoded = _make_decoded_tecx()
    rich = RichTecxTranscoder().decode(decoded, decode_context)
    assert rich.tech_cost_settings[TechId.RESTORATION].tech_id == TechId.RESTORATION


def test_it_decodes_uses_default_settings_as_bool(decode_context):
    decoded = _make_decoded_tecx(uses_default=1)
    rich = RichTecxTranscoder().decode(decoded, decode_context)
...
def test_it_decodes_costs_correctly(decode_context):
    decoded = _make_decoded_tecx(mineral=200, gas=100, time=3600, energy=75)
    rich = RichTecxTranscoder().decode(decoded, decode_context)
