...
from richchk.transcoder.richchk.transcoders.rich_tile_transcoder import (
    RichTileTranscoder,
)
...
@pytest.fixture
def real_decoded_tile() -> DecodedTileSection:
    with open(
        CHK_SECTION_FILE_PATHS[DecodedTileSection.section_name().value], "rb"
    ) as f:
        chk_binary_data = f.read()
    return ChkTileTranscoder().decode(chk_binary_data)
...
def test_it_decodes_expected_tile_count(real_decoded_tile, rich_decode_context):
    rich_tile = RichTileTranscoder().decode(
        real_decoded_tile,
        rich_chk_decode_context=rich_decode_context,
    )
    assert len(rich_tile.tiles) == 128 * 128


def test_integration_it_decodes_and_encodes_back_without_changing_data(
    real_decoded_tile, rich_decode_context, rich_encode_context
):
    rich_tile = RichTileTranscoder().decode(
        real_decoded_tile,
        rich_chk_decode_context=rich_decode_context,
    )
    actual_decoded_tile = RichTileTranscoder().encode(
        rich_tile,
        rich_chk_encode_context=rich_encode_context,
    )
    rich_tile_again = RichTileTranscoder().decode(
        actual_decoded_tile,
        rich_chk_decode_context=rich_decode_context,
    )
    assert rich_tile == rich_tile_again
    assert actual_decoded_tile == real_decoded_tile
