...
from richchk.model.chk.upgx.decoded_upgx_section import DecodedUpgxSection
...
from richchk.transcoder.richchk.transcoders.rich_upgx_transcoder import (
    RichUpgxTranscoder,
)
...
def test_it_decodes_correct_number_of_settings(decode_context):
    decoded = _make_decoded_upgx()
    rich = RichUpgxTranscoder().decode(decoded, decode_context)
...
def test_it_decodes_bw_upgrade_ids_as_keys(decode_context):
    decoded = _make_decoded_upgx()
    rich = RichUpgxTranscoder().decode(decoded, decode_context)
    assert (
        rich.upgrade_cost_settings[UpgradeId.ARGUS_JEWEL].upgrade_id
        == UpgradeId.ARGUS_JEWEL
    )


def test_it_decodes_uses_default_settings_as_bool(decode_context):
    decoded = _make_decoded_upgx(uses_default=1)
    rich = RichUpgxTranscoder().decode(decoded, decode_context)
    assert (
...
def test_it_decodes_costs_correctly(decode_context):
    decoded = _make_decoded_upgx(base_mineral=200, base_gas=50)
    rich = RichUpgxTranscoder().decode(decoded, decode_context)
    assert (
